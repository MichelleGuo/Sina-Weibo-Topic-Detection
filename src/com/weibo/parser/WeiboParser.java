...
public class WeiboParser {
	...
	public static List<Element> getGoalContent(Document doc) {
		...
		if (Constants.CHECK_WEIBO_NUM) {
			...
			int weiboNum = Integer.parseInt(element.text().substring(
	...
	public static Weibo parse(Element weiboEl, String poster) {
		...
		//weibo.setAuthorUrl(weiboEl.attr("href"));
		//weibo.setAuthorName(weiboEl.attr("title"));

		try {
			int subDivsSize = subDivs.size();
			weibo.setMid(weiboEl.attr("id").substring(2));
			weibo.setPoster(poster);
			weibo.setDateString(StrUtils.parseDate(weiboEl.getElementsByClass("ct")
					.get(0).text().split("来自")[0]));
			
			//mark!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			String[] authorname = weiboEl.attr("title").substring(0).split("的微博");
			weibo.setAuthorName(authorname);
			
			//weibo.setAuthorName(weiboEl.getElementsByClass("ctt").get(0).text().split(";")[0]);
	...
	// 将抓取的微博信息保存至本地文件
	public static void createFile(List<Element> weiboItems, String uid) {
		//String userID = Utils.getUserIdFromUrl(urlPath);
		String userID = uid; 
		Log.debug("createFile_将抓取的微博信息保存至本地文件");
...
//					.prepareStatement("INSERT INTO weibo (accountID, weiboID, content, postTime) VALUES (?, ?, ?, ?)");
			
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO content_weibo (uid, mid, content, addtime) VALUES (?, ?, ?, ?)");
			for (Element el : weiboItems) {
				Weibo weibo = WeiboParser.parse(el, userID);
				ps.setString(1, userID);
				ps.setString(2, weibo.getMid());
				ps.setString(3, weibo.getContent());
				ps.setString(4, weibo.getDateString());
				ps.executeUpdate();
			}
			ps.close();
